...
```{r}
library(dplyr)
Obama_16 <- data_frame(line = 1:70, text = text) %>%
...
afinn <- Obama_16 %>% 
  ...
  group_by(index = line %/% 70) %>% 
  summarise(sentiment_df = sum(score)) %>% 
  mutate(method_df = "AFINN")
```
```{r}
bing_and_nrc <- bind_rows(Obama_16 %>% 
...
```{r}
sum(sentiment_line$net_sentiment)
```
